...
/// SLH-DSA-SHAKE-256s digital signatures (NIST standard (FIPS 205), hash-based, 256-bit security).
///
/// FIPS 205 standard. Uses SHAKE internally, optimized for small signatures.
/// Hash-based security — no lattice or code-based assumptions.
///
/// ```swift
/// // Generate a signing key
/// let signer = try SLHDSAPureSHAKE256s.PrivateKey()
///
/// // Sign something
/// let sig = try signer.signature(for: messageData)
///
/// // Anyone with the public key can verify
/// let pub = try SLHDSAPureSHAKE256s.PublicKey(rawRepresentation: signerPublicKeyData)
...
/// let loaded = try SLHDSAPureSHAKE256s.PrivateKey(
