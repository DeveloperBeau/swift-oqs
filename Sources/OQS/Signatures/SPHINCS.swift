...
public enum SPHINCSSHA2128fSimple: Sendable {
    ...
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try sigGenerateKeyPair(algorithm: SPHINCSSHA2128fSimple.algorithmName)
            self.rawRepresentation = kp.secretKey
            self.publicKey = PublicKey(unchecked: kp.publicKey)
        }

        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try sigExpectedKeyLengths(algorithm: SPHINCSSHA2128fSimple.algorithmName)
        ...
        public func signature(for data: Data) throws -> Data {
            try sigSign(algorithm: SPHINCSSHA2128fSimple.algorithmName, message: data, secretKey: rawRepresentation)
        }
    }
    ...
    public struct PublicKey: Sendable {
        ...
        public func isValidSignature(_ signature: Data, for data: Data) throws -> Bool {
            try sigVerify(algorithm: SPHINCSSHA2128fSimple.algorithmName, message: data, signature: signature, publicKey: rawRepresentation)
        }
    }
}
...
public enum SPHINCSSHA2128sSimple: Sendable {
    ...
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try sigGenerateKeyPair(algorithm: SPHINCSSHA2128sSimple.algorithmName)
            self.rawRepresentation = kp.secretKey
            self.publicKey = PublicKey(unchecked: kp.publicKey)
        }

        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try sigExpectedKeyLengths(algorithm: SPHINCSSHA2128sSimple.algorithmName)
            guard rawRepresentation.count == lengths.secretKey else {
                throw OQSError.invalidKeySize(expected: lengths.secretKey, actual: rawRepresentation.count)
            }
            guard publicKeyRepresentation.count == lengths.publicKey else {
        ...
        public func signature(for data: Data) throws -> Data {
            try sigSign(algorithm: SPHINCSSHA2128sSimple.algorithmName, message: data, secretKey: rawRepresentation)
        }
    }
    ...
    public struct PublicKey: Sendable {
        ...
        public func isValidSignature(_ signature: Data, for data: Data) throws -> Bool {
            try sigVerify(algorithm: SPHINCSSHA2128sSimple.algorithmName, message: data, signature: signature, publicKey: rawRepresentation)
        }
    }
}
...
public enum SPHINCSSHA2192fSimple: Sendable {
    ...
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try sigGenerateKeyPair(algorithm: SPHINCSSHA2192fSimple.algorithmName)
            self.rawRepresentation = kp.secretKey
            self.publicKey = PublicKey(unchecked: kp.publicKey)
        }

        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try sigExpectedKeyLengths(algorithm: SPHINCSSHA2192fSimple.algorithmName)
            guard rawRepresentation.count == lengths.secretKey else {
                throw OQSError.invalidKeySize(expected: lengths.secretKey, actual: rawRepresentation.count)
            }
            guard publicKeyRepresentation.count == lengths.publicKey else {
        ...
        public func signature(for data: Data) throws -> Data {
            try sigSign(algorithm: SPHINCSSHA2192fSimple.algorithmName, message: data, secretKey: rawRepresentation)
        }
    }
    ...
    public struct PublicKey: Sendable {
        ...
        public func isValidSignature(_ signature: Data, for data: Data) throws -> Bool {
            try sigVerify(algorithm: SPHINCSSHA2192fSimple.algorithmName, message: data, signature: signature, publicKey: rawRepresentation)
        }
    }
}
...
public enum SPHINCSSHA2256fSimple: Sendable {
    ...
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try sigGenerateKeyPair(algorithm: SPHINCSSHA2256fSimple.algorithmName)
            self.rawRepresentation = kp.secretKey
            self.publicKey = PublicKey(unchecked: kp.publicKey)
        }

        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try sigExpectedKeyLengths(algorithm: SPHINCSSHA2256fSimple.algorithmName)
            guard rawRepresentation.count == lengths.secretKey else {
                throw OQSError.invalidKeySize(expected: lengths.secretKey, actual: rawRepresentation.count)
            }
            guard publicKeyRepresentation.count == lengths.publicKey else {
        ...
        public func signature(for data: Data) throws -> Data {
            try sigSign(algorithm: SPHINCSSHA2256fSimple.algorithmName, message: data, secretKey: rawRepresentation)
        }
    }
    ...
    public struct PublicKey: Sendable {
        ...
        public func isValidSignature(_ signature: Data, for data: Data) throws -> Bool {
            try sigVerify(algorithm: SPHINCSSHA2256fSimple.algorithmName, message: data, signature: signature, publicKey: rawRepresentation)
        }
    }
}
...
public enum SPHINCSSHAKE128fSimple: Sendable {
    ...
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try sigGenerateKeyPair(algorithm: SPHINCSSHAKE128fSimple.algorithmName)
            self.rawRepresentation = kp.secretKey
            self.publicKey = PublicKey(unchecked: kp.publicKey)
        }

        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try sigExpectedKeyLengths(algorithm: SPHINCSSHAKE128fSimple.algorithmName)
            guard rawRepresentation.count == lengths.secretKey else {
                throw OQSError.invalidKeySize(expected: lengths.secretKey, actual: rawRepresentation.count)
            }
            guard publicKeyRepresentation.count == lengths.publicKey else {
        ...
        public func signature(for data: Data) throws -> Data {
            try sigSign(algorithm: SPHINCSSHAKE128fSimple.algorithmName, message: data, secretKey: rawRepresentation)
        }
    }
    ...
    public struct PublicKey: Sendable {
        ...
        public func isValidSignature(_ signature: Data, for data: Data) throws -> Bool {
            try sigVerify(algorithm: SPHINCSSHAKE128fSimple.algorithmName, message: data, signature: signature, publicKey: rawRepresentation)
        }
    }
}
...
public enum SPHINCSSHAKE256fSimple: Sendable {
    ...
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try sigGenerateKeyPair(algorithm: SPHINCSSHAKE256fSimple.algorithmName)
            self.rawRepresentation = kp.secretKey
            self.publicKey = PublicKey(unchecked: kp.publicKey)
        }

        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try sigExpectedKeyLengths(algorithm: SPHINCSSHAKE256fSimple.algorithmName)
            guard rawRepresentation.count == lengths.secretKey else {
                throw OQSError.invalidKeySize(expected: lengths.secretKey, actual: rawRepresentation.count)
            }
            guard publicKeyRepresentation.count == lengths.publicKey else {
        ...
        public func signature(for data: Data) throws -> Data {
            try sigSign(algorithm: SPHINCSSHAKE256fSimple.algorithmName, message: data, secretKey: rawRepresentation)
        }
    }
    ...
    public struct PublicKey: Sendable {
        ...
        public func isValidSignature(_ signature: Data, for data: Data) throws -> Bool {
            try sigVerify(algorithm: SPHINCSSHAKE256fSimple.algorithmName, message: data, signature: signature, publicKey: rawRepresentation)
        }
    }
}
