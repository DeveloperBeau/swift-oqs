...
/// eFrodoKEM-976-SHAKE key encapsulation (NIST Round 3 alternate, ~192-bit security, SHAKE-based PRF).
///
...
/// secrets. For long-lived keys use the salted ``FrodoKEM976SHAKE``.
///
/// ```swift
/// // Alice generates a key pair
/// let alice = try EFrodoKEM976SHAKE.PrivateKey()
///
/// // Bob gets Alice's public key and creates a shared secret
/// let pub = try EFrodoKEM976SHAKE.PublicKey(rawRepresentation: alicePublicKeyData)
...
/// let loaded = try EFrodoKEM976SHAKE.PrivateKey(
///     rawRepresentation: saved,
///     publicKeyRepresentation: alice.publicKey.rawRepresentation
/// )
/// ```
public enum EFrodoKEM976SHAKE: Sendable {
    static let algorithmName = "eFrodoKEM-976-SHAKE"

    /// An eFrodoKEM-976-SHAKE private (decapsulation) key.
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try kemGenerateKeyPair(algorithm: EFrodoKEM976SHAKE.algorithmName)
        ...
        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try kemExpectedKeyLengths(algorithm: EFrodoKEM976SHAKE.algorithmName)
        ...
        public func decryptSharedSecret(_ ciphertext: Data) throws -> SharedSecret {
            let ss = try kemDecapsulate(algorithm: EFrodoKEM976SHAKE.algorithmName, ciphertext: ciphertext, secretKey: rawRepresentation)
            return SharedSecret(rawRepresentation: ss)
        }
    }

    /// An eFrodoKEM-976-SHAKE public (encapsulation) key.
    public struct PublicKey: Sendable {
        ...
        public func generateSharedSecret() throws -> SharedSecretResult {
            let result = try kemEncapsulate(algorithm: EFrodoKEM976SHAKE.algorithmName, publicKey: rawRepresentation)
