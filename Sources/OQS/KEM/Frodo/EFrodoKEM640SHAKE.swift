...
/// eFrodoKEM-640-SHAKE key encapsulation (NIST Round 3 alternate, ~128-bit security, SHAKE-based PRF).
///
...
/// secrets. For long-lived keys use the salted ``FrodoKEM640SHAKE``.
///
/// ```swift
/// // Alice generates a key pair
/// let alice = try EFrodoKEM640SHAKE.PrivateKey()
///
/// // Bob gets Alice's public key and creates a shared secret
/// let pub = try EFrodoKEM640SHAKE.PublicKey(rawRepresentation: alicePublicKeyData)
...
/// let loaded = try EFrodoKEM640SHAKE.PrivateKey(
///     rawRepresentation: saved,
///     publicKeyRepresentation: alice.publicKey.rawRepresentation
/// )
/// ```
public enum EFrodoKEM640SHAKE: Sendable {
    static let algorithmName = "eFrodoKEM-640-SHAKE"

    /// An eFrodoKEM-640-SHAKE private (decapsulation) key.
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try kemGenerateKeyPair(algorithm: EFrodoKEM640SHAKE.algorithmName)
        ...
        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try kemExpectedKeyLengths(algorithm: EFrodoKEM640SHAKE.algorithmName)
        ...
        public func decryptSharedSecret(_ ciphertext: Data) throws -> SharedSecret {
            let ss = try kemDecapsulate(algorithm: EFrodoKEM640SHAKE.algorithmName, ciphertext: ciphertext, secretKey: rawRepresentation)
            return SharedSecret(rawRepresentation: ss)
        }
    }

    /// An eFrodoKEM-640-SHAKE public (encapsulation) key.
    public struct PublicKey: Sendable {
        ...
        public func generateSharedSecret() throws -> SharedSecretResult {
            let result = try kemEncapsulate(algorithm: EFrodoKEM640SHAKE.algorithmName, publicKey: rawRepresentation)
